...
class Stream:
    ...
    def __init__(self, ffprobe_info: dict):
        self.ffinfo = dict_difference(flatten_dict_join(ffprobe_info), Stream.UNAVAILABLE_DATA)
    ...
    def key_handling(key: StreamKeyType) -> str:
        ...
        assertTrue(isinstance(key, str), "Can't handle key '{}' of type {}", key, type(key))
        return key
    ...
    def is_default(self, key: StreamKeyType) -> bool:
        ...
        return (k in self.ffinfo) and (k in Stream.DEFAULT_VALUES) and (Stream.DEFAULT_VALUES[k] == self.ffinfo[k])
...
class FutureStream(Stream):
    ...
    def __init__(self, index: int, codec: Optional[str] = None, stream_type: Optional[StreamType] = None, encoder: Optional[Encoder] = None, **kwargs) -> None:
        ...
        if encoder is None and (codec is None or stream_type is None):
            raise ValueError("Parameters required: either 'codec' and 'stream_type', or 'encoder' (can't all be None)")
