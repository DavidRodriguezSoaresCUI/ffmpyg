...
from ffile import (FfprobeInfoKey, FutureMediaFile, FutureStream, MediaFile,
                   Stream, StreamCriteria, StreamType)
...
class FfmpegOptions:
    ...
    def __init__(self, loglevel: str = 'info', hide_banner: bool = False, y_overwrite: bool = False, n_overwrite: bool = False, no_stats: bool = False, more: Optional[List[str]] = None) -> None:
        self.cfg = {
    ...
    def assertValueInRange(cls, name: str, value: Any) -> Any:
        ...
        assertTrue(name in cls.ACCEPTED_VALUES, "No entry '{}' in FfmpegOptions.ACCEPTED_VALUES !", name)
        assertTrue(value in cls.ACCEPTED_VALUES[name], "Value '{}' not in {} !", value, cls.ACCEPTED_VALUES[name])
        return value
...
class FfmpegInput:
    ...
    def __init__(self, file: MediaFile, fix_fps: Union[int,float,None] = None, more: Optional[List[str]] = None) -> None:
        ''' Expects a file and modifiers; for now only fps fixing with  '''
        self._file = file
        assertTrue(fix_fps is None or (isinstance(fix_fps, (int, float)) and fix_fps > 0), "Invalid value for fix_fps '{}' must be a int or float and be a non-zero positive value !", fix_fps)
...
class FilterComplexNode:
    ...
    def __init__(self, inputs: List[Union['FilterComplexNode',str]], complex_filter: str) -> None:
        self.inputs = inputs
    ...
    def compute_filter(self, **kwargs) -> Union[str, None]:
        ...
        if self.next is None and not kwargs:
            ...
            _inputs = random_words(n=len(self.inputs), length=FilterComplexNode.NAME_LEN, used_words=used_names)
        ...
        _inputs = random_words(n=len(self.inputs), length=FilterComplexNode.NAME_LEN, used_words=kwargs['used_names'])
    ...
    def to_string(self, inputs: List[str], output: Optional[str] = None) -> str:
        ...
        _inputs = [_in if isinstance(_in, str) else inputs[idx] for idx,_in in enumerate(self.inputs)] # get actual self.inputs instead of generated if possible
        return FilterComplexNode.make_tags(_inputs) + self.complex_filter + ('' if output is None else FilterComplexNode.make_tags([output]))


def build_ffmpeg_command(inputs: List[FfmpegInput], stream_mapping: Dict[Stream, FutureStream], output: Union[str,Path], ffmpeg: Union[str,Path] = 'ffmpeg', options: Optional[FfmpegOptions] = None, filter_complex: Optional[FilterComplexNode] = None, extra: Optional[list] = None) -> Tuple[Command, Optional[FutureMediaFile]]:
    ...
    if stream_mapping is not None:
        in_stream_sorted_by_out_stream_idx = sorted(
            stream_mapping.keys(),
            key=lambda in_stream: stream_mapping[in_stream].idx
        )
        for in_stream in in_stream_sorted_by_out_stream_idx:
            out_stream = stream_mapping[in_stream]
            input_file_idx = min(filter(
                lambda x: x is not None,
                [idx for idx,i in enumerate(inputs) if i.contains_stream(in_stream)])
            )
            stream_command = ['-map', f"{input_file_idx}:{in_stream.idx}"]
            if out_stream.encoder is not None:
                stream_command.extend([f"-c:{out_stream.idx}", out_stream.encoder.encoder])
...
def build_mapping(_input: MediaFile, rules: Dict[StreamCriteria, Encoder]) -> Dict[Stream, FutureStream]:
    ''' Builds stream mapping from encoder rules
    '''
    # Ensure there is no more than one default criteria
    is_default_criteria = lambda criteria: criteria.codec is None and criteria.codec_type is None
    default_rules = [criteria for criteria in rules.keys() if is_default_criteria(criteria)]
    assertTrue(len(default_rules) < 2, "More than one default rule is defined: {}", default_rules)

    # Map streams by index in input file
    mapped_streams: Dict[int,StreamCriteria] = {}
    # Note: default criteria are visited last
    for stream_criteria in sorted(rules.keys(), key=lambda c: 1 if is_default_criteria(c) else 0):
        ...
        for in_stream in _input.get_streams(stream_criteria):
            if in_stream.idx in mapped_streams:
                if not is_default_criteria(stream_criteria):
                    LOG.warning("Warning: Stream %s matched non-default criteria %s but already matched previous criteria %s !", in_stream, stream_criteria, mapped_streams[in_stream.idx])
                continue
        ...
        if before_count == len(mapped_streams):
            LOG.warning("Info: criteria {stream_criteria} didn't match any stream for file {_input}")
            sys.exit()
    ...
    for out_idx, in_idx in enumerate(sorted(mapped_streams.keys())):
        ...
        codec = in_stream.get(FfprobeInfoKey.CODEC) if encoder.is_copy else encoder.codec
        stream_type = StreamType(in_stream.get(FfprobeInfoKey.STREAM_TYPE))
        mapping[in_stream] = FutureStream(index=out_idx, codec=codec, stream_type=stream_type, encoder=encoder)
...
def main2():
    ...
    _filter = FilterComplexNode(
        inputs=[
            FilterComplexNode(inputs=['0:v:0'], complex_filter="{drawText('reference')}"),
