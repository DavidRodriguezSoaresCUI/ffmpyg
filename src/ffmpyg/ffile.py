...
class MediaFile:
    ...
    def __init__(self, path: Path) -> None:
        self.path = path
        self.streams = {} if (path is None or not path.is_file()) else {
            stream.get(FfprobeInfoKey.INDEX): stream
            for stream in [
                Stream(stream_info)
                for stream_info in file_stream_info(path)
            ]
        }
    ...
    def get_streams(self, criteria: StreamCriteria) -> List[Stream]:
        ...
        return [
            ...
            if all(
                d_criteria[c] is None or stream.get(c) == (d_criteria[c].value if isinstance(d_criteria[c], StreamType) else d_criteria[c])
    ...
    def valid(self) -> bool:
        ...
        return len(self.streams) > 0 and \
            list(sorted(self.streams.keys())) == list(range(max(self.streams.keys()) + 1))
...
class FutureMediaFile(MediaFile):
    ...
    def __init__(self, path: Path) -> None:
        assertTrue(path is not None and not path.exists(), "Path must be non-null and not exist: path={}", path)
        super().__init__(path)
    ...
    def add_stream(self, stream: FutureStream) -> 'FutureMediaFile':
        ...
        assertTrue(stream.idx == next_stream_idx, "Expected FutureStream index to be {}, not {}", next_stream_idx, stream.idx)
